...
const Checkout = () => {
    const initialValues={nombre: "", email: "", email2: "", DNI: "", cellphone: "", address: ""}
...
//Aqui se validan para que se ingresen correctamente los diferentes elementos de los formularios, sin errores.
    ...
    const validate = (values)=>{
        ...
        if (!values.DNI){
            errors.DNI="Ingrese por favor su DNI.";
        }
    ...
    const checkForm = (e) => {
        ...
        createOrdenCompra(cliente,totalPrice(), new Date().toISOString().slice(0,10)).then(ordenCompra => {
            getOrdenCompra(ordenCompra.id).then(item => {
                toast.success(`¡Muchas gracias por compra en Memorinvent, su orden de Compra es ${item.id}`)
                emptyCart()              
                navigate("/")
            }).catch(error => {
